...
$query = "SELECT * FROM ANIMAL WHERE ESPECIE='Canina' AND DIVULGAR_COMO='GAAR' AND (ADOTADO <>'Óbito' AND ADOTADO NOT LIKE '%Adotado%') ORDER BY LAR_TEMPORARIO,NOME_ANIMAL ASC";
...
if ($reccount <> '0') {
    ...
    <div class='starter-template'>
        <center>
            ...
            <h3>RELATÓRIO SEMANAL DOS CÃES </h3><br>
    ...
    $subject = "[GAAR Campinas] Relatório semanal dos cães";
    ...
    $mail->AddAddress($to);
    $to_cpc = "thaise.piculi@gmail.com";
    $mail->AddBCC($to_cpc);
    
    $querymail = "SELECT EMAIL FROM VOLUNTARIOS WHERE CPC='SIM'";
    ...
    if (!$mail->send()) {
        $log_file_msg .="[enviorelatoriocpc.php] Erro no envio de relatório da cpc: ".$mail->ErrorInfo." às ".$horaatu."\n";
        $fp = fopen($log_file, 'a');//opens file in append mode  
        fwrite($fp, $log_file_msg);  
    } else {
        $log_file_msg .="[enviorelatoriocpc.php] Envio de relatório da cpc para ".$lista_email." às ".$horaatu."\n"; 
        $fp = fopen($log_file, 'a');//opens file in append mode  
        fwrite($fp, $log_file_msg);  
        echo $bodytext;
    } 
