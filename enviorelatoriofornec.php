...
$mes_atu =  date('m');
$ano_atu = date('Y');
$querystatus = "SELECT * FROM VENDAS_PRODUTOS where DT_VENDA >= '".$ano_atu."-".$mes_ant."-01 00:00:00' and DT_VENDA <= '".$ano_atu."-".$mes_ant."-31 99:99:99' and PRODUTO like '%Caneca%' ORDER BY DT_VENDA ASC";
...
if ($reccount <> '0') {
    ...
    while ($fetchstatus = mysqli_fetch_row($resultstatus)) {
        	...
        	$queryprod = "SELECT PRECO_CUSTO FROM CONTROLE_ESTOQUE WHERE ID = '$idprod'";
        	$resultprod = mysqli_query($connect,$queryprod);
        	
        	echo "<br> query prod: ".$queryprod;
    ...
    if ($result){
        ...
        echo "<br>Erro no envio";
    }
    
    $subject = "[GAAR Campinas] Pagamento das vendas online de ".$mes_ant."/".$ano_atu."";
