    ...
    if(mysqli_errno($connect) == '0'){
    	...
    	switch ($evento) {
    	  case 'Feira de adoção':
        		...
        		$log_file_msg="[cadastroevento.php] ".$subject." inserida na tabela EVENTOS às ".$horaatu."\n";
          ...
          case 'Bazar':
        		...
        		$log_file_msg="[cadastroevento.php] ".$subject." inserido na tabela EVENTOS às ".$horaatu."\n";
        ...
        while ($fetchmailop = mysqli_fetch_row($selectmailop)) {
            ...
            $mail->AddAddress($to);
            if (!$mail->send()) {
                $log_file_msg="[cadastroevento.php] Erro de envio de evento: Mailer Error: " . $mail->ErrorInfo." às ".$horaatu."\n";
                $fp = fopen($log_file, 'a');//opens file in append mode  
                fwrite($fp, $log_file_msg);  
            } else {
                $log_file_msg="[cadastroevento.php] Envio de evento para ".$to." às ".$horaatu."\n";
                $fp = fopen($log_file, 'a');//opens file in append mode  
                fwrite($fp, $log_file_msg);  
            }
            $mail->clearAddresses();
        ...
        window.location.href='formcadastroevento.php'</script>";
    }else{ 
    	echo "Insert code: ".$insert;
    	echo "Mensagem de erro: ".mysql_error(). "SQL Error: ".mysql_errno();
    	echo "Erro ao cadastrar <br><br>";
    	echo "<a href='formcadastroevento.php'>Voltar</a>";
      /*echo"<script language='javascript' type='text/javascript'>
