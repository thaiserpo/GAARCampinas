...
if($login == "" || $login == null){
    ...
    if ($usuario == $login){
        $query = "UPDATE VOLUNTARIOS SET STATUS_APROV='Inativo' WHERE USUARIO = '$usuariovol'";
        $desativa = mysqli_query($connect,$query);
        
        if(mysqli_errno($connect) == '0'){
            echo"<script language='javascript' type='text/javascript'>
                                        alert('Usuário desativado com sucesso');window.location
